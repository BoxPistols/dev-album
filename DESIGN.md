...
### 採用: a11y を CI ゲート化（axe-core / 3 テーマ）
- 理由: 「ビルドが通る ≠ 正しく描画される」。コントラスト退行を PR 段階で止める。
- 対象: 代表ページ + コードブロック（補間トークンを含む）ページ。critical/serious 違反ゼロを不変条件とする。
...
- E2E は現状 **a11y スペックのみ CI 化**している。フル E2E（プレビュー/threejs 等）は安定性の都合でローカル/手動実行のまま。全 E2E の CI 化は次段の課題。
- サイドバー現在地以外にも、nav/chrome 側に color-contrast の別課題が残る可能性がある（発見時は本ファイルに追記）。
