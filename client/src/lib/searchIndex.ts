...
export const searchIndex: Record<string, string[]> = {
  ...
  '/react/storybook/intro': [
    ...
    'デザイナーとエンジニア', 'エコシステム', '画面構成', 'Storybook 9', 'Storybook 8',
  ],
  ...
  // ===== claude-code-manual =====
  '/claude-code': ['対象となるエンジニア', '学習のロードマップ', 'ガイドの特徴', '準備はよろしいですか'],
  '/claude-code/getting-started/why-claude-code': ['ターミナルファーストのAIエージェント', '他のAIコーディングツールとの違い', 'エージェンティック開発の実践', 'Copilot', 'Cursor'],
  '/claude-code/claude-intro/claude-code-intro': ['設計コンセプト', '主要な機能カテゴリ', '信頼と安全性のモデル', 'IDE統合', '導入手順', 'キーボードショートカット', 'CLI'],
  '/claude-code/claude-intro/install-setup': ['インストール', '認証の設定', 'プロジェクトの初期化', '基本的な起動オプション', 'npm', 'API キー'],
  '/claude-code/claude-intro/slash-commands': ['コア操作コマンド', 'セッション管理', 'コスト・コンテキスト監視', '設定・診断', '開発ワークフロー', 'その他のユーティリティ', 'カスタムコマンド', 'クイック入力プレフィックス', '/help', '/init', '/clear', '/compact', '/cost'],
  '/claude-code/claude-core/context-management': ['CLAUDE.md メモリ階層', '@import構文とルール管理', '自動メモリ', '.claudeignore', '秘匿情報の保護', 'コンテキスト使用量の監視'],
  '/claude-code/claude-core/security-permissions': ['承認フロー', 'Human-in-the-loop', 'パーミッションモード', 'パーミッションルール', 'サンドボックス', '設定ファイルの優先順位', 'allow', 'deny'],
  '/claude-code/claude-core/token-optimization': ['コスト監視コマンド', '自動コンパクション', 'Effort Level', '推論強度', 'コスト削減のベストプラクティス', 'トークン'],
  '/claude-code/claude-core/extended-thinking': ['モデルファミリーと選択基準', 'Effort Level', '推論深度の制御', '拡張思考', 'Extended Thinking', 'タスク別の推奨設定', 'Opus', 'Sonnet', 'Haiku'],
  '/claude-code/mcp/mcp-setup': ['アーキテクチャの理解', 'MCPサーバの追加', 'MCPスコープ', 'MCP管理コマンド一覧', 'チームでのMCP共有', 'Model Context Protocol'],
  '/claude-code/mcp/mcp-practical': ['デザイン → コード', 'ドキュメント参照', 'コード理解・ナビゲーション', 'ブラウザ自動化', '監視・バックエンド連携', '実践的なプロンプト例', 'HTTP MCPとOAuth認証', 'Figma', 'Sentry', 'Puppeteer'],
  '/claude-code/agent-extensions/subagents': ['マルチエージェント・オーケストレーション', 'ビルトインサブエージェント', 'カスタムエージェントの定義', 'バックグラウンド実行と監視', '並列処理'],
  '/claude-code/agent-extensions/custom-skills': ['Skills', 'スキル', 'カスタムスラッシュコマンド', 'Hooks', 'ライフサイクルフック', '使い分けガイド'],
  '/claude-code/ai-coding-agents/gemini-cli': ['Gemini CLI', 'Google', 'オープンソース', 'Apache 2.0', '無料枠', 'GEMINI.md', 'MCP', 'Google 検索グラウンディング', 'npx @google/gemini-cli', 'YOLO モード'],
  '/claude-code/ai-coding-agents/openai-codex': ['OpenAI Codex', 'Codex CLI', 'ChatGPT', 'AGENTS.md', 'サンドボックス', '承認モード', 'クラウドタスク', 'codex exec', 'Rust', 'IDE 拡張'],
  '/claude-code/ai-coding-agents/github-copilot': ['GitHub Copilot', 'コード補完', 'Copilot Chat', 'Copilot CLI', 'コーディングエージェント', 'エージェントモード', 'copilot-instructions.md', 'プレミアムリクエスト', 'AI クレジット', 'モデル選択'],
  '/claude-code/ai-coding-agents/amazon-q-developer': ['Amazon Q Developer', 'AWS', 'Kiro', 'Builder ID', 'IAM Identity Center', 'コード変換', 'Java アップグレード', 'エージェンティックコーディング', 'Bedrock', 'セキュリティスキャン'],
  '/claude-code/ai-coding-agents/choosing-tools': ['ツール比較', '使い分け', '選定基準', '料金比較', 'エコシステム', 'Claude Code', 'Gemini CLI', 'Codex', 'Copilot', 'Amazon Q'],
  '/claude-code/claude-core/context-engineering': ['コンテキストエンジニアリング', 'context window', 'トークン管理', '/clear', '/compact', '/rewind', '/btw', '@import', 'Repository Impact Map', 'prompt cache', '1-hour cache', 'subagent context'],
  '/claude-code/best-practices/harness-engineering': ['ハーネスエンジニアリング', 'harness', 'Agent = Model + Harness', 'Constrain Inform Verify Correct', 'guides sensors', 'Managed Agents', 'meta-harness', 'brain hands session', 'Martin Fowler'],
  '/claude-code/multi-ai/agent-docs': ['CLAUDE.md', 'AGENTS.md', 'ARCHITECTURE.md', '3 層構造', 'アーキテクチャ', '意思決定', 'ADR', 'SSOT', '@import', 'ツール非依存', '.claude/rules', 'paths', 'compact'],
  '/claude-code/multi-ai/design-md': ['DESIGN.md', 'Google Labs', 'Stitch', 'デザイントークン', 'front matter', 'WCAG', 'コントラスト比', 'Tailwind', 'DTCG', 'lint', 'デザインシステム'],
  '/claude-code/cmux/cmux-intro': ['cmux', 'Ghostty', 'macOS', 'ネイティブアプリ', 'GUI', '通知リング', '垂直タブ', 'ビルトインブラウザ', 'tmux との比較', 'ソケット API', 'ワークスペース'],
  '/claude-code/cmux/cmux-setup': ['cmux インストール', 'brew', 'Homebrew', 'CLI シンボリックリンク', 'Claude Code Hooks', 'stop hook', '通知設定', 'マルチエージェント', 'ワークスペース', 'manaflow-ai', 'ペイン分割'],
  '/claude-code/cmux/agent-teams': ['Agent Teams', 'teammate', 'cmux claude-teams', 'ネイティブ split', 'マルチエージェント', '並列タスク', 'レビュー担当', '実装担当', 'tmux 比較', 'ロール分担'],
  '/claude-code/cmux/browser-api': ['ビルトインブラウザ', 'Scriptable API', 'ax-snapshot', 'アクセシビリティツリー', 'click', 'fill', 'evaluate', 'JavaScript', 'dev サーバー検証', 'エージェント操作'],
  '/claude-code/cmux/worktrees': ['git worktree', 'cmux worktree', '並列ブランチ', '作業ツリー分離', 'feature branch', 'hotfix', 'craigsc/cmux', 'manaflow-ai/cmux', 'worktree prune', 'worktree remove'],
  '/claude-code/reference/troubleshooting': ['よくある問題', 'FAQ', 'トラブルシューティング'],
  '/claude-code/reference/claude-cheatsheet': ['スラッシュコマンド', 'コア操作', 'セッション管理', '設定・ワークフロー', 'CLI 起動オプション', 'キーボードショートカット', 'CLAUDE.md の構成', 'パーミッションモード'],
  '/claude-code/best-practices/effective-workflows': ['検証可能な指示を与える', 'Explore → Plan → Implement → Commit', '具体的なコンテキストを提供する', 'セッション管理', 'Fast Mode の活用', 'よくあるアンチパターン', 'ワークフロー設計'],
  '/claude-code/best-practices/spec-driven-dev': ['SDD', 'Spec-Driven Development', '仕様駆動開発', 'TDD との違い', 'SDD のワークフロー', 'CLAUDE.md と仕様ファイルの連携'],
  '/claude-code/best-practices/testing-debugging': ['テスト駆動の開発フロー', 'Writer/Reviewer パターン', 'エラーの根本原因を特定するプロンプティング', 'Subagents を使ったコードレビュー', 'スクリーンショットベースの UI デバッグ', 'ファンアウト: 大規模移行での並列処理'],
  '/claude-code/hooks-advanced/hooks-guide': ['Hooks とは', 'Hook イベント一覧', '3つの Hook タイプ', 'Matcher によるフィルタリング', '入出力の仕様', '設定ファイルの配置スコープ', 'PreToolUse', 'PostToolUse', 'command', 'prompt', 'agent'],
  '/claude-code/hooks-advanced/hooks-recipes': ['デスクトップ通知', 'ファイル編集後の自動フォーマット', '保護ファイルへの編集ブロック', 'コンパクション後のコンテキスト再注入', 'ツール使用の監査ログ', 'Prompt Hook: タスク完了チェック', 'Agent Hook: テスト実行による検証'],
  '/claude-code/ci-cd/github-actions': ['claude-code-action の概要', 'セットアップ手順', 'トリガーイベント', '主要パラメータ', 'セキュリティと権限', '実践パターン', 'GitHub Actions'],
  '/claude-code/ci-cd/headless-mode': ['パイプモード', '自動化スクリプト', '権限の自動承認', 'サンドボックス環境', 'SDK によるプログラム的な利用', 'ヘッドレス実行の主要オプション', '-p', '--dangerously-skip-permissions'],
  '/claude-code/ide-agent-teams/ide-integration': ['VS Code 連携', 'JetBrains 連携', 'ターミナル統合のセットアップ', 'デスクトップアプリ', '実践的な使い分け', 'IDE 共通の操作ヒント'],
  '/claude-code/ide-agent-teams/agent-orchestration': ['マルチエージェントの概要', 'Agent Teams の表示モード', 'Git ワークツリーによる分離', 'Subagents とマルチインスタンスの違い', 'エージェント間の情報共有', 'マルチエージェントのベストプラクティス'],
  '/claude-code/ide-agent-teams/plugins-ecosystem': ['プラグインの概要', 'プラグインの管理', 'プラグインの発見', 'カスタムスラッシュコマンド', 'MCP エコシステムとの連携', 'プラグイン開発の基本'],
  '/claude-code/multi-ai/multi-ai-coexistence': ['なぜマルチ AI 戦略が必要か', '主要 AI コーディングツールの比較', 'ハイブリッドアーキテクチャの設計', '機能分担パターン', 'スキルの共通化', '段階的導入ガイド'],
  '/claude-code/multi-ai/single-source-of-truth': ['なぜ SSOT が必要か', '階層的ルール管理', 'SSOT 運用のベストプラクティス', 'シングルソースオブトゥルース'],
