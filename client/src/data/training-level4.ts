...
export const level4Challenges: TrainingChallenge[] = [
  ...
  return (
    <nav style={{ background: '#1a1a2e', padding: '12px 20px', position: 'relative' }}>
      ...
      {mobile && open && (
        <div
          style={{
            display: '___', // ← ここを埋める（モバイルメニューの表示方法）
  ...
  const theme = dark
    ? { bg: '___', text: '___', card: '#16213e', accent: '#e94560' } // ← ここを埋める（ダーク時の背景色と文字色）
  ...
  const validate = (field, value) => {
    if (field === 'name') return value.trim().length < 2 ? '___' : ''; // ← ここを埋める（名前のエラーメッセージ）
  ...
  return (
    <div style={{ maxWidth: 400, margin: '40px auto', fontFamily: 'sans-serif' }}>
      ...
      <form onSubmit={(e) => e.preventDefault()} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <button
          type="submit"
          disabled={___} // ← ここを埋める（ボタンを無効にする条件）
          style={{
            padding: '12px 0',
            borderRadius: 6,
            border: 'none',
  ...
  const handleScroll = () => {
    ...
    setScroll({
      ...
      right: el.scrollLeft < el.___ - el.___ - 1, // ← ここを埋める（スクロール可能幅の計算に使う2つのプロパティ）
    });
  };
  ...
  return (
    <div style={{ fontFamily: 'sans-serif', padding: 40, textAlign: 'center' }}>
      ...
      {open && (
        ...
        >
          <div
            onClick={(e) => e.___()}  // ← ここを埋める（モーダル内クリックがbackdropに伝播しないようにする）
            style={{
              background: '#fff',
              borderRadius: 12,
              padding: 32,
    ...
    <div style={{ maxWidth: 500, margin: '40px auto', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <button
          ...
          disabled={___} // ← ここを埋める（Backボタンを無効にする条件）
          style={{
            padding: '10px 24px',
            borderRadius: 6,
        ...
        <button
          onClick={() => setStep(step + 1)}
          disabled={___} // ← ここを埋める（Nextボタンを無効にする条件）
          style={{
            padding: '10px 24px',
            borderRadius: 6,
            border: 'none',
  ...
  const filtered = items.___((___)  => // ← ここを埋める（フィルタリングのメソッドとコールバック引数）
  ...
  const loadMore = () => {
    ...
    setTimeout(() => {
      ...
      ___(___); // ← ここを埋める（読み込み完了時にloadingをfalseに戻す）
    }, 800);
  };
  ...
  const handleDrop = (e) => {
    e.preventDefault();
    setDragging(false);
    const dropped = Array.from(e.___.___).map((f) => ({ // ← ここを埋める（ドロップされたファイルの取得方法）
      name: f.name,
  ...
  return (
    <div style={{ padding: 32, fontFamily: 'sans-serif', maxWidth: 960, margin: '0 auto' }}>
      ...
      <div
        style={{
          display: 'grid',
          gridTemplateColumns: 'repeat(___, minmax(___px, ___))', // ← ここを埋める（自動カラム数・最小幅・最大幅）
          gap: 20,
        }}
      >
        {cards.map((card) => (
