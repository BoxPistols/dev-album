...
export const level2Challenges: TrainingChallenge[] = [
  ...
  return (
    <div style={{ padding: 40, display: 'flex', justifyContent: 'center' }}>
      <div style={{ position: '___', width: 64, height: 64 }}>{/* ← ここを埋める */}
        <div
          style={{
            width: 64,
            height: 64,
  ...
  return (
    ...
    hints: [
      "onClick で setActive(i) を呼んでアクティブタブを切り替える",
      "コンテンツ表示には contents[active] でインデックスアクセスする",
      "marginBottom: -2 でタブの下線と親の borderBottom を重ねる",
    ],
    keywords: ["setActive(i)", "contents[active]"],
  },
  ...
  return (
    ...
    hints: [
      "onClick で setOpen(!open) を呼んで開閉を切り替える",
      "{open && (...)} で条件付きレンダリングする",
      "boxShadow で浮き上がって見える効果を付ける",
    ],
    keywords: ["setOpen(!open)", "open"],
  },
          ...
          <React.Fragment key={crumb}>
            ...
            <span
              style={{
                ...
                cursor: i < crumbs.length - 1 ? '___' : 'default', // ← ここを埋める
                textDecoration: i < crumbs.length - 1 ? '___' : 'none', // ← ここを埋める
              }}
            >
              {crumb}
            </span>
  ...
  return (
    ...
    hints: [
      "リンクに見せるには cursor: pointer を設定する",
      "textDecoration: underline で下線を付ける",
      "最後の要素だけスタイルを変えて現在地を示す",
    ],
    keywords: ["pointer", "underline"],
  },
      ...
      {plans.map((plan) => (
        <div
          ...
          style={{
            ...
            transform: plan.highlighted ? '___' : 'none', // ← ここを埋める
  ...
  return (
    ...
    hints: [
      ...
      "flexDirection: column でカード内を縦に並べる",
    ],
    keywords: ["flex", "scale(1.05)", "column"],
  },
  ...
  return (
    <div style={{ padding: 80, display: 'flex', justifyContent: 'center' }}>
      <span
        ...
        onMouseEnter={() => ___} // ← ここを埋める
        onMouseLeave={() => ___} // ← ここを埋める
      >
        <span
          style={{
  ...
  return (
    ...
    hints: [
      "onMouseEnter で setShow(true) を呼ぶ",
      "onMouseLeave で setShow(false) を呼ぶ",
      "transform: translateX(-50%) と left: 50% で中央揃え",
    ],
    keywords: ["setShow(true)", "setShow(false)"],
  },
  ...
  return (
    <div style={{ padding: 40, display: 'flex', justifyContent: 'center' }}>
      <div style={{ position: '___', display: 'inline-block', fontSize: 32 }}>{/* ← ここを埋める */}
  ...
  return (
    ...
    hints: [
      ...
      "内側の div に width を percent + '%' で指定する",
      "linear-gradient でグラデーション色にすると見栄えが良い",
    ],
    keywords: ["hidden", "percent + '%'"],
  },
  ...
  const addTag = (e) => {
    if (e.key === '___' && input.trim() !== '') { // ← ここを埋める
      e.preventDefault();
  ...
  const removeTag = (tag) => {
    setTags(tags.filter((t) => ___)); // ← ここを埋める
  };
  ...
  return (
    ...
    hints: [
      "e.key === 'Enter' で Enter キーを検知する",
      "tags.filter((t) => t !== tag) で対象タグを除外する",
      "flexWrap: wrap でタグが増えても折り返す",
    ],
    keywords: ["Enter", "t !== tag"],
  },
    ...
    <div style={{ padding: 40, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
      <div style={{ display: 'flex', gap: 4, fontSize: 32 }}>
        {[1, 2, 3, 4, 5].map((n) => (
          ...
          >
            {___} {/* ← ここを埋める: 塗りつぶし★か白抜き☆を条件で切り替え */}
          </span>
        ))}
      </div>
  ...
  return (
    ...
    hints: [
      "n <= (hover || rating) で塗りつぶすかどうかを判定する",
      "条件に一致すれば ★、そうでなければ ☆ を表示する",
      "hover 用の state で、クリック前にホバープレビューを実現する",
    ],
    keywords: ["n <= (hover || rating) ? '★' : '☆'"],
  },
];
