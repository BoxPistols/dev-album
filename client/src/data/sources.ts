...
export const CURATED_SOURCES: Source[] = [
  ...
  },
  {
    ...
    title: "typesafe-sdk（TypeSafe AI 公式 Python SDK）— PyPI に掲載された README",
  ...
  },
  {
    ...
    title: "@typesafe-ai/sdk 0.6.0（TypeSafe AI 公式 JavaScript SDK）の型定義と README（npm から取得して展開）",
    kind: "measured",
    verifiedAt: "2026-09-19",
    reproduce:
      "npm pack @typesafe-ai/sdk@0.6.0 && tar xzf typesafe-ai-sdk-0.6.0.tgz && cat package/README.md package/dist/index.d.mts",
    note:
      "README は「Install the SDK (Node.js 20 or newer)」「Set `TYPESAFE_API_KEY` in your environment, then create and use the client」「Answer types are inferred from your questions.」と書く。index.d.mts は NoulResponse の noul を「Probability of a yes answer, from zero to one.」、ChoiceResponse の confidence を「Reported confidence in the selected label.」、ScoreResponse の score を「Expected score, which may fall between integer rubric levels.」、SystemOneRequestPayload を「Request body for `POST /v1/systemone`」、baseURL の既定を「https://api.typesafe.ai」、defaultModel の既定を「jev-latest」、timeout を「Timeout per attempt in milliseconds, without a total retry budget. Default: 10000.」、RetryPolicy の maxRetries を「Default: 2.」、httpStatuses を「Default: 408, 429, and 500–599.」、dangerouslyAllowBrowser を「Allow browser use, exposing the API key to page users. Default: false.」と定義している。package.json の engines は node >=20、publish 日時は 2026-09-15T18:17:19Z（npm レジストリの time フィールド）",
    usedBy: ["/ai-ml/jev/jev-overview", "/ai-ml/jev/jev-account", "/ai-ml/jev/jev-setup", "/ai-ml/jev/jev-primitives", "/ai-ml/jev/jev-state-design", "/ai-ml/jev/jev-app-ideas", "/ai-ml/jev/jev-ledger-app", "/ai-ml/jev/jev-copycheck-app", "/ai-ml/jev/jev-feedback-app", "/ai-ml/jev/jev-clinic-app", "/ai-ml/jev/jev-leads-app", "/ai-ml/jev/jev-advanced"],
  },
  {
    id: "typesafe-sdk-python-0-7-0-schemas",
    title: "typesafe-sdk 0.7.0（TypeSafe AI 公式 Python SDK）の OpenAPI 生成スキーマ（PyPI から取得して展開）",
    kind: "measured",
    verifiedAt: "2026-09-19",
    reproduce:
    ...
    note:
      "models.py の先頭に「generated by datamodel-codegen: filename: https://api.typesafe.ai/openapi.json」とあり、API の OpenAPI 定義から生成されている。NoulAnswer.noul は「Probability of a yes answer or a true statement, from 0 to 1. Values near 1 favor yes or true, values near 0 favor no or false, and values near 0.5 indicate uncertainty.」、ChoiceAnswer.choice は「The name of the choice with the highest probability among the question's criteria.」、ChoiceAnswer.probabilities は「values sum to approximately 1」、ScoreAnswer.score は「Expected score: the probability-weighted average of the rubric levels. May fall between integer levels.」、ScoreQuestion.criteria は「Each description's position determines its score, starting at zero.」、Usage.output_tokens は「Output tokens are currently free of charge.」、SystemOneRequest.model は「Available names are returned by GET /v1/models.」、ModelMetadata.release_date の例は「2026-09-15」と書かれている。constants.py は DEFAULT_BASE_URL = https://api.typesafe.ai、DEFAULT_MODEL = jev-latest、DEFAULT_TIMEOUT = 10.0（秒）。pyproject の requires-python は >=3.10",
    usedBy: ["/ai-ml/jev/jev-overview", "/ai-ml/jev/jev-account", "/ai-ml/jev/jev-setup", "/ai-ml/jev/jev-primitives", "/ai-ml/jev/jev-state-design", "/ai-ml/jev/jev-app-ideas", "/ai-ml/jev/jev-ledger-app", "/ai-ml/jev/jev-copycheck-app", "/ai-ml/jev/jev-feedback-app", "/ai-ml/jev/jev-clinic-app", "/ai-ml/jev/jev-leads-app", "/ai-ml/jev/jev-advanced"],
  },
];
