...
< etag: "a1b2c3d4"`}
  ...
  --data '{"files":["https://example.com/style.css"]}'`}
            ...
            <InfoBox
              type="warning"
              title="仕様と実測のズレ：パージは「即時」ではない"
            >
              ドキュメント上はパージ後に古いキャッシュが消えますが、
              実測では世界中の PoP
              すべてに反映が行き渡るまで数秒〜十数秒の幅があります。
              理由は、パージ命令自体がエッジ網へ伝播するのに時間がかかるためです。
              「パージ＝全ユーザーに一瞬で反映」と思い込むと、検証時に古い内容を見て混乱します。
