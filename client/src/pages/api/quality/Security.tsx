...
const owaspRisks = [
  {
    ...
    description:
      "ID を指定するエンドポイントで所有者チェックを怠り、認証済みユーザーが他人のリソースを読み書きできてしまう。報告件数が最も多い類型。",
  },
  {
    id: "API2",
    title: "認証の不備",
  ...
  return res.json(order);
          ...
          <section>
            <Quiz
              ...
              explanation="BOLA は、ID を受け取るエンドポイントでリソース単位の所有者・権限チェックを省いた結果、認証は通っている正規ユーザーが他人のオブジェクトにアクセスできてしまう脆弱性です。OWASP API Security Top 10 で最上位に位置づけられる、最も報告の多い類型です。対策はサーバ側でリソースの所有者を毎回検証することです。"
            />
          </section>
