...
export default function JevStateDesign() {
    ...
    <div className="min-h-screen bg-background page-enter">
      <div className="max-w-4xl mx-auto px-4 md:px-8 py-12">
        ...
        <p className="text-lg text-muted-foreground mb-8 leading-relaxed">
          Jev の使い方は「state を渡して質問する」だけですが、何を state
          に入れ、質問をどう切るかで答えの安定性が変わります。
          このページでは設計の指針と、1
          リクエストで複数の質問を評価する形、そして失敗したときの SDK
          の振る舞いを扱います。
        </p>
        ...
        >
          <p>
            LLM のプロンプト設計に相当する部分です。ただし Jev
            には「文体を指定する」「例を見せて真似させる」といった
            余地が少なく、代わりに「判断に必要な情報を state
            に過不足なく入れる」「質問を 1 判断ずつに切る」ことが効きます。
          </p>
        </WhyNowBox>
        ...
        <div className="space-y-12 mt-8">
          ...
          <section>
            ...
            <p className="text-muted-foreground mb-4 leading-relaxed">
              state は API スキーマで「The content all questions in this request
  ...
  history: ["2 tickets in the last 30 days"],
            ...
            <div className="rounded-xl border border-border bg-card p-6 mt-4">
              ...
              <ul className="space-y-2 text-sm text-muted-foreground">
                ...
                <li className="flex items-start gap-2">
                  <span className="text-primary font-bold mt-0.5">-</span>
                  <span>
                    質問側の instructions からキー名で参照できる（「Use the
            ...
            <InfoBox type="warning" title="個人情報は入れる前に落とす">
              ...
              LMOps の講座で扱った PII フィルタリングと同じ扱いです。
            </InfoBox>
          </section>
          ...
          <section>
            ...
            <p className="text-muted-foreground mb-4 leading-relaxed">
              ...
              判断ごとに質問を分け、名前を付けて 1 リクエストにまとめます。
            </p>
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-border">
                  ...
                  <tr className="bg-card">
                    ...
                    <td className="py-3 px-4 text-muted-foreground">
                      「請求」「ログイン」「速度」を別々の noul にする（choice
                      は 1 つしか選べない）
                    </td>
                  </tr>
                </tbody>
...
// 型: answers.category.choice は "billing" | "technical" | "sales"
// 型: answers.frustration.probabilities のキーは "0" | "1" | "2"`}
            />
            <p className="text-muted-foreground mt-4 leading-relaxed">
              複数の入力（チケット 10 件）を 1 回で評価したい場合は、state
              を配列にして、質問名に番号を含める形が公式 SDK
              の型の範囲で書けます。 1
              リクエストに入れられる質問数の上限は、手元で確認できた SDK と
          ...
          <section>
            ...
            <p className="text-muted-foreground mb-4 leading-relaxed">
              ...
              が入ります。 スキーマは input_tokens を「billable input
              tokens」、output_tokens を「currently free of
              charge」と説明しています（執筆時点）。 つまりコストは state と
              questions の大きさで決まります。state
...
for (const m of models) {
  console.log(m.name, m.release_date, m.description);
          ...
          <section>
            ...
            <p className="text-muted-foreground mb-4 leading-relaxed">
              JavaScript SDK の既定値を型定義から起こします。Python SDK
              も同じ考え方で、タイムアウトは 10.0 秒です。
            </p>
            <div className="overflow-x-auto rounded-xl border border-border">
              <table className="w-full text-sm">
                <thead>
                ...
                <tbody className="divide-y divide-border">
                  <tr className="bg-card">
                    ...
                    <td className="py-3 px-4 text-muted-foreground">
                      1 試行あたり 10000 ミリ秒。リトライ全体の合計上限は無い
                    </td>
                  </tr>
                  <tr className="bg-card">
                  ...
                  <tr className="bg-card">
                    ...
                    <td className="py-3 px-4 text-muted-foreground">
                      500 ミリ秒から倍々、最大 5000 ミリ秒。ジッタ 0.25
                    </td>
                  </tr>
                  <tr className="bg-card">
...
try {
  ...
  if (err instanceof AuthenticationError) {
    ...
    // 422: リクエストの形が不正。err.body に検証エラーの位置（loc）が入る
  } else if (err instanceof RateLimitError) {
    // 429: SDK のリトライを使い切った。err.retryAfterMs を見てキューに戻す
  ...
  throw err;
          ...
          <section>
            ...
            <CodingChallenge
              title="複数タグを noul で分ける"
              description="1 つの choice に詰め込まれていた「請求 / ログイン / 速度」を、同時に当てはまり得るタグとして noul 3 つに分けてください。___ を埋めます。"
              preview={false}
...
const tags = Object.entries(answers)
  ...
  .map(([name]) => name);`}
              hints={[
                "同時に当てはまり得る条件は、choice ではなく noul を条件ごとに置きます",
                "noul の答えは a.noul に 0〜1 で入ります。しきい値と比較してタグに採用します",
              ]}
          ...
          <section>
            ...
            <Quiz
              question="Jev の呼び出しがタイムアウトで失敗したとき、適切な扱いは？"
              options={[
                { label: "確率 0 として「いいえ」と同じ処理に流す" },
                {
                  label:
                    "「判断が得られなかった」状態として別経路（人の確認など）に回す",
                  correct: true,
                },
                { label: "確率 1 として「はい」と同じ処理に流す" },
                { label: "無視して次の入力に進む" },
              ]}
              ...
              links={[
                {
                  ...
                  description: "SDK の応答型がリンクしている概念ページ。",
                },
                {
                  title: "@typesafe-ai/sdk（npm）",
                  url: "https://www.npmjs.com/package/@typesafe-ai/sdk",
                  description:
                    "型定義（dist/index.d.mts）に RetryPolicy と RequestOptions の既定値が書かれている。",
                },
              ]}
            />
