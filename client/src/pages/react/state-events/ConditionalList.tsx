...
`}
          ...
          <section>
            <Quiz
              ...
              explanation="index を key にすると、項目の追加・削除・並べ替え時に、React が要素を正しく特定できなくなります。例えば先頭に項目を追加すると、各項目の index が 1 つずつズレます。key が index のままだと React は「同じ位置に同じ要素がある」と見なして state と DOM をその位置に残すため、入力中のテキストが別の項目に付いたままになります。静的なリストならば index でも問題ありません。"
            />
          </section>
