...
- 追加ページ（stepは既存末尾の直後・連番）:
  ...
  7. `/ai-ml/jev/jev-ledger-app` サンプルアプリ1: 家計簿の自動仕分け（日常生活。Next.js Route Handler + React、基本形）
  8. `/ai-ml/jev/jev-copycheck-app` サンプルアプリ2: UI文言チェッカー（デザイナー向け。指針をstateで渡す3分岐、フェイルクローズ）
