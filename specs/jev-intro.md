...
1. `client/src/lib/navigation.ts` に SectionInfo 1 件 + PageInfo 10 件を追加
2. `client/src/lib/navigation.test.ts` の総ページ数・ai-ml のページ数を更新
3. `client/src/App.tsx` に lazy import + Route を 10 件追加
...
- 正常系: 新セクションがサイドバーに表示され、10 ページが 3 テーマで描画・相互リンクする。TOP の「最新のお知らせ」に 10 件が出る
